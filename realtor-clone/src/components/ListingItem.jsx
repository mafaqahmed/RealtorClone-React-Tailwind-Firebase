...
export default function ListingItem({ id, listing }) {
  return (
    <li
      className="bg-white relative rounded overflow-hidden shadow-md hover:shadow-lg transition duration-150 ease-in-out m-2"
    ...
    >
      ...
      <div className="flex items-center mt-3 ml-2">
        ...
        <div className="ml-1">
          <p className="text-gray-600 font-medium text-sm truncate">{listing.address}</p>
        </div>
      </div>
      ...
      <div className="ml-2 mt-4 mb-4 flex items-center">
        <div className="mr-4">
          <p className="font-bold text-xs">{listing.bedrooms > 1 ? `${listing.bedrooms} beds` : '1 Bed'}</p>
        </div>
        <div>
          <p className="font-bold text-xs">{listing.bethrooms > 1 ? `${listing.bethrooms} bath` : '1 Bath'}</p>
        </div>
      </div>
    </li>
  );
